...
final class PresentCardTransitionDriver {
    ...
    init(params: PresentCardAnimator.Params, transitionContext: UIViewControllerContextTransitioning, baseAnimator: UIViewPropertyAnimator) {
        ...
        let screens: (home: HomeViewController, cardDetail: DealDetailsViewController) = (
            ctx.viewController(forKey: .from)! as! HomeViewController,
            ctx.viewController(forKey: .to)! as! DealDetailsViewController
        )
        ...
        let topTemporaryFix = screens.cardDetail.dealImageView.topAnchor.constraint(equalTo: cardDetailView.topAnchor, constant: 0)
